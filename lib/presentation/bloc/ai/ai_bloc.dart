...
class AIBloc extends Bloc<AIEvent, AIState> {
  ...
  AIBloc({required this.aiRepository}) : super(AIInitial()) {
    ...
    on<GenerateImageByNameRequested>(_onGenerateImageByNameRequested);
    on<FetchGeneratedPhotosRequested>(_onFetchGeneratedPhotosRequested);
  ...
  Future<void> _onGenerateImageByNameRequested(GenerateImageByNameRequested event, Emitter<AIState> emit) async {
    emit(AILoading());
    final result = await aiRepository.generateImageByName(
      photoId: event.photoId,
      recommendations: event.recommendations,
    );
    result.fold(
      (failure) => emit(AIError(failure.message)),
      (request) => emit(AIGenerationRunning(request.requestId)),
    );
  }

  Future<void> _onFetchGeneratedPhotosRequested(FetchGeneratedPhotosRequested event, Emitter<AIState> emit) async {
    emit(AILoading());
    final result = await aiRepository.getGeneratedPhotos(
      userId: event.userId,
      requestId: event.requestId,
    );
    result.fold(
      (failure) => emit(AIError(failure.message)),
      (photos) => emit(GeneratedPhotosLoaded(photos)),
    );
  }
